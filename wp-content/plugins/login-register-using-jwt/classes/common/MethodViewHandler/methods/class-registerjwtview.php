				...
				<table width="120%">
					<tr>
						...
						<td><input type="text" id="mo_jwt_api_key" name="mo_jwt_api_key" value="<?php echo esc_attr( $this->api_key ); ?>" readonly>
							<button type="button" class="button button-secondary" onclick="MoJWTcopyApiKey()">Copy</button>
							<?php
							if ( $this->jwt_versi ) {
								?>
								<button type="button" class="button button-secondary" onclick="MoJWTgenerateNewApiKey()">Generate New Token</button>
								<?php
							}
							?>
						</td>
